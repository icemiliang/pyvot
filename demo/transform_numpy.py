...
# ------- run WM -------- #
vot = Vot(data_p=data_p, data_e=data_e,
...
tick = time.time()
ot_reg.cluster(reg_type='transform', reg=20, max_iter_p=5)
tock = time.time()
print("total running time : {0:.4f} seconds".format(tock-tick))
...
# ----- plot before ----- #
cp = [utils.color_blue, utils.color_red]
cp = [cp[int(label)] for _, label in np.ndenumerate(ot_reg.label_p)]
...
plt.scatter(vot.data_e[:, 0], ot_reg.data_e[:, 1], marker='.', color=ce, zorder=2)
plt.scatter(p_coor_after[:, 0], p_coor_after[:, 1], marker='o', facecolors='none', linewidth=2, color=cp, zorder=3)
