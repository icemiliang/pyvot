...
vot = VOT(y.clone(), [x.clone()], label_y=labels, verbose=False)
...
# ------- run RWM ------- #
vot_reg = VOTREG(y.clone(), [x.clone()], label_y=labels, verbose=False)
print("running regularized Wasserstein clustering...")
tick = time.time()
vot_reg.map(reg_type='potential', reg=0.02, max_iter_y=5)
tock = time.time()
print("total running time : {0:g} seconds".format(tock-tick))
...
plt.savefig("potential_torch.png")
plt.show()
